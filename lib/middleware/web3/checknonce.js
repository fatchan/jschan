	...
	checkNonceParamConverter(req, res, async () => {
		...
		if (res.locals.isWeb3) {
			errors = await checkSchema([
				{ result: globalWeb3 === true && boardWeb3 === true, expected: true, error: __('Web3 logins disabled') },
		...
		try {
			if (res.locals.isWeb3) {
				...
				res.locals.recoveredAddress = (await web3EthAccountsRecover(`Nonce: ${nonce}`, signature)).toLowerCase();
