...
module.exports = async (req, res, next) => {
	...
	checkNonceParamConverter(req, res, async () => {
		...
		res.locals.web3Signed = req.body.signature && req.body.signature.length > 0;
		...
		try {
			...
			//TODO: global option to not set recoveredAddress purely from eth account login
			if (!res.locals.recoveredAddress
				&& res.locals.user && res.locals.user.web3 === true) {
				res.locals.recoveredAddress = res.locals.user.username;
			}
